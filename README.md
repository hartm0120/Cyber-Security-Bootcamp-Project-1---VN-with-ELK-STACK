...
- Wget-DOS script - a for/while nested loop script was created to run from the Jump-Box VM which indexed the DVWA website on the web severs to increase traffic and simulate a Denial of Service (DoS) attack. Using the the metrics dashboard on the Kibana app to we could visualise the network traffic entering and exiting the web servers, which illustrated incoming traffic significantly increased.
